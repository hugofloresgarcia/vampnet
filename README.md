...
**control masks**: unfortunately, `ctrl_masks` are the opposite: a `1` means the code is given as conditioning, and a `0` means the code is generated. this comes from the fact that the control mask is multiplicative, and a `1` means "keep this ctrl", and a `0` means "no control is present".
