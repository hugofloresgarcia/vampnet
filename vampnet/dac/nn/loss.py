...
class CREPELoss(nn.Module):
    
    def forward(self, x: AudioSignal, y: AudioSignal):
        from .signal import Signal
        x = Signal(x.audio_data, x.sample_rate)
        y = Signal(y.audio_data, y.sample_rate)
        xppg = x.crepe_ppg(_raw=True)
        yppg = y.crepe_ppg(_raw=True)

        return F.l1_loss(xppg, yppg)
