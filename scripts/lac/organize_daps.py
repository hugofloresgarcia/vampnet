...
def process(
    ...
    if dataset == "daps":
        ...
        audio_files = [
            x for x in util.find_audio(dataset_path) if daps_subset in str(x) and "breaths" not in str(x)
        ]
    ...
    with util.chdir(dataset_path):
        for k, v in splits.items():
            ...
            for _v in tqdm.tqdm(v):
                tgt_path = pathlib.Path(str(_v).replace(str(dataset_path), str(dataset_path / k)))
